...
const fixAllPermissions = async () => {
  try {
    ...
    for (const user of users) {
      ...
      // 3. Set correct permissions based on role
      ...
      const correctPermissions = rolePermissions[newRole];
      
      if (correctPermissions) {
        updates.permissions = correctPermissions;
        needsUpdate = true;
        console.log(`🔐 Setting ${newRole} permissions for ${user.username}`);
      }
    ...
    finalUsers.forEach((user, index) => {
      ...
      console.log(`   User Management Access: ${user.permissions?.users?.view ? 'YES' : 'NO'}`);
      console.log(`   Can Create/Edit/Delete: ${user.permissions?.garage?.create ? 'YES' : 'NO'}`);
    });
